...
class DriverDatasetSR(data.Dataset):
    def __init__(self, LR_root,GT_root,scale_factor,isTrain):
        ...
        self.GT_transforms=self.get_default_img_transform()
    ...
    def __getitem__(self, index):
        ...
        gt = self.GT_transforms(Image.open(gt_path))
...
if __name__ == '__main__':
    ...
    GT_root="../data/temp/test_processed_input"
    scale_factor=2
    d=DriverDatasetSR( LR_root=LR_root,GT_root=GT_root,scale_factor=scale_factor,isTrain=True)
    n=d.__getitem__(0)
    print(n[0].shape)
    print(n[1].shape)
    print(torch.sum(n[1]))